...
namespace TNet
{
  ...
  */
  ...
  template<class T> inline std::istream& operator >> (std::istream &istream, std::vector<T> &vec){
    size_t sz;
    istream >> sz; if(!istream.good()){ KALDI_ERR << "Error reading to vector of [something]: stream bad\n";  }
  ...
  template<class T> inline std::istream& operator >> (std::istream &istream, std::list<T> &lst){
    size_t sz;
    istream >> sz; if(!istream.good()){ KALDI_ERR << "Error reading to list of [something]: stream bad\n";  }
    lst.resize(sz);
    typename std::list<T>::iterator it;
    for(it = lst.begin(); it != lst.end(); it++)
      istream >> *it;
